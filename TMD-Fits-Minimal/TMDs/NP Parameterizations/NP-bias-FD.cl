...
inline float8 NP_f_func(float x, float b, __constant Params_Struct* params)
{
    ...
    float inv_denom = 1/sqrtf(1.0f + l0*x*x*b2);

    float SNP_u   = expf(-shape_u*inv_denom*b2);
    float SNP_ub  = expf(-shape_ub*inv_denom*b2);
    float SNP_d   = expf(-shape_d*inv_denom*b2);
    float SNP_db  = expf(-shape_db*inv_denom*b2);
    float SNP_sea = expf(-shape_sea*inv_denom*b2);

    float SNP_ze = -g2 * b2/sqrtf(1.0 + b2/(BNP*BNP));
