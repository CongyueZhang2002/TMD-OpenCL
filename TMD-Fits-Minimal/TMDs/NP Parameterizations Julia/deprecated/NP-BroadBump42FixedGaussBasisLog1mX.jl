...
function DNP_broad42_fixedbasis_log1mx_func(b_64::Float64)
    BNP = Float32(NP_BNP)
    c0 = Float32(NP_c0)
    c1 = Float32(NP_c1)
...
function NP_f_func(x_64::Float64, b_64::Float64)
    ...
    logx = log(x)
    log1mx = log(1f0 - x)
    base = lambda1 * xbar + lambda3 * xxbar + lambda4 * logx + lambda5 * log1mx

    u = (logx - FIXED_LOGX0) / FIXED_SIGX
    ...
    SNP_ζ = -Float32(DNP_broad42_fixedbasis_log1mx_func(b_64))
