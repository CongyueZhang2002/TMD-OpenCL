...
RESULTS_DIR = FITS_DIR / "log_basis_42_fixedshape_results"
RESULTS_DIR.mkdir(exist_ok=True)

BASE_RESULT = FITS_DIR / "log_basis_42_targeted_results" / "xbar_quad_logpair_fixedbump.json"
DEFAULT_CANDIDATES = ["xbar_quad_logpair_leg2", "xbar_quad_logpair_leg23"]
...
def _render_kernel(spec: CandidateSpec) -> str:
    extra_struct = "  float a5"
    leg_expr = "a5 * P2"
    extra_a6_load = ""
    if spec.kernel_variant == "xbar_quad_logpair_leg23":
        extra_struct = "  float a5, a6"
        leg_expr = "a5 * P2 + a6 * P3"
        extra_a6_load = "  const float a6 = p->a6;\n"

    return f"""#define bmax 1.1229189f
#define xh   0.1f
#define Q0   1.0f
#define LOGX_MIN (-6.907755278982137f)
#define LOGX_MAX (-0.10536051565782628f)
...
{{
  ...
  const float a5 = p->a5;
{extra_a6_load}  const float BNP = p->BNP;
  const float c0 = p->c0;
  const float c1 = p->c1;
  ...
  float u = 2.f * (logx - LOGX_MIN) / (LOGX_MAX - LOGX_MIN) - 1.f;
  u = clampf(u, -1.f, 1.f);
  float P2 = 0.5f * (3.f * u * u - 1.f);
  float P3 = 0.5f * (5.f * u * u * u - 3.f * u);

  float shape = lambda1 * xbar + lambda2 * xxbar + lambda3 * logx + lambda4 * log1mx + {leg_expr};
...
class FixedShapeFitSession(Art23FamilyFitSession):
    def candidate_start_points(self) -> list[np.ndarray]:
        starts = super().candidate_start_points()
        extra_anchors = {
            "a5": [-1.5, -0.8, -0.3, 0.3, 0.8, 1.5],
            "a6": [-1.0, -0.4, 0.4, 1.0],
        }
        for pname, vals in extra_anchors.items():
            if pname not in self.free_param_names:
                continue
            idx = self.free_param_names.index(pname)
            lo = self.lower_bounds[idx]
            hi = self.upper_bounds[idx]
            for val in vals:
                start = self.theta0.copy()
                start[idx] = np.clip((val - lo) / (hi - lo), 0.0, 1.0)
                starts.append(start)
...
def candidate_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            name="xbar_quad_logpair_leg2",
            fit_name="BroadBump42XbarQuadLogPairLeg2",
            np_name="NP-BroadBump42XbarQuadLogPairLeg2.cl",
            param_names=["lambda1", "lambda2", "lambda3", "lambda4", "alpha", "a5", "BNP", "c0", "c1"],
            initial_params=[base[0], base[1], base[2], base[3], 1.0, 0.0, base[8], base[9], base[10]],
            bounds=common_bounds,
            frozen_indices=[4],
            kernel_variant="xbar_quad_logpair_leg2",
        ),
        CandidateSpec(
            name="xbar_quad_logpair_leg23",
            fit_name="BroadBump42XbarQuadLogPairLeg23",
            np_name="NP-BroadBump42XbarQuadLogPairLeg23.cl",
            param_names=["lambda1", "lambda2", "lambda3", "lambda4", "alpha", "a5", "a6", "BNP", "c0", "c1"],
            initial_params=[base[0], base[1], base[2], base[3], 1.0, 0.0, 0.0, base[8], base[9], base[10]],
            bounds=leg23_bounds,
            frozen_indices=[4],
            kernel_variant="xbar_quad_logpair_leg23",
        ),
    ]
...
def run_candidate(spec: CandidateSpec, maxfun: int) -> Path:
    session = FixedShapeFitSession(spec)
...
def summarize_results(specs: dict[str, CandidateSpec]) -> pd.DataFrame:
    ...
    rows = [
        {
            "candidate": "xbar_quad_logpair_fixedbump",
            "fit_name": "BroadBump42XbarQuadLogPairFixedBump",
