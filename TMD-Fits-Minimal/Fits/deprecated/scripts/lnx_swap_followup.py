...
RESULTS_DIR = FITS_DIR / "lnx_swap_followup_results"
...
def _render_kernel(spec: CandidateSpec) -> str:
    if spec.kernel_variant == "swap_sqrtlog":
  ...
  float xbar = 1.f - x;
  float logterm = sqrtf(fmax(-log(x), 1e-7f));
  float base = lambda1 * xbar + lambda2 * x + lambda4 * logterm;
"""
    elif spec.kernel_variant == "swap_softlogfit":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float logxs;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  ...
  float xbar = 1.f - x;
  float xs = expf(logxs);
  float logterm = log(x + xs) - log(1.f + xs);
  float base = lambda1 * xbar + lambda2 * x + lambda4 * logterm;
"""
    elif spec.kernel_variant == "swap_powquarter":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  ...
  x = clampf(x, 1e-7f, 1.f - 1e-7f);
  float xbar = 1.f - x;
  float logterm = powr(fmax(x, 1e-7f), -0.25f) - 1.f;
  float base = lambda1 * xbar + lambda2 * x + lambda4 * logterm;
"""
    else:
...
def followup_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            name="swap_sqrtlog",
            fit_name="SwapSqrtLog",
            np_name="NP-SwapSqrtLog.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="swap_sqrtlog",
        ),
        CandidateSpec(
            name="swap_softlogfit",
            fit_name="SwapSoftLogFit",
            np_name="NP-SwapSoftLogFit.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "logxs", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, logx0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
                ...
                (0.15, 2.5),
                (-2.0, 2.0),
                (np.log(1e-5), np.log(0.3)),
                (0.4, 4.5),
                (0.0, 0.25),
                (0.0, 0.25),
            ],
            frozen_indices=[],
            kernel_variant="swap_softlogfit",
        ),
        CandidateSpec(
            name="swap_powquarter",
            fit_name="SwapPowQuarter",
            np_name="NP-SwapPowQuarter.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
            ...
            frozen_indices=[],
            kernel_variant="swap_powquarter",
        ),
    ]
    ...
    rows = [
        {
            ...
            "cms_highmass_mean_signed_first3": ref_metrics["cms_highmass_mean_signed_first3"],
            "zlike_mean_signed_first3": ref_metrics["zlike_mean_signed_first3"],
        }
    ]
    for name in specs:
        ...
        for pname, value in zip(result["param_names"], best_full):
            row[pname] = value
        rows.append(row)
    df = pd.DataFrame(rows).sort_values(["chi2dN_total", "highE_mean_absdev_first3"], ascending=[True, True]).reset_index(drop=True)
