...
RESULTS_DIR = FITS_DIR / "log_basis_42_targeted_results"
RESULTS_DIR.mkdir(exist_ok=True)

BASE_RESULT = FITS_DIR / "log_basis_42_results" / "xbar_quad_logpair_bump.json"

DEFAULT_CANDIDATES = [
    "centered_quad_logpair_bump",
    "xbar_quad_logpair_fixedbump",
]
...
def _render_kernel(spec: CandidateSpec) -> str:
    if spec.kernel_variant == "centered_quad_logpair":
        base_expr = "lambda1 + lambda2 * z2 + lambda3 * logx + lambda4 * log1mx"
    elif spec.kernel_variant == "xbar_quad_logpair":
        base_expr = "lambda1 * xbar + lambda2 * xxbar + lambda3 * logx + lambda4 * log1mx"
    else:
...
def candidate_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            name="centered_quad_logpair_bump",
            fit_name="BroadBump42CenteredQuadLogPair",
            np_name="NP-BroadBump42CenteredQuadLogPair.cl",
            param_names=["lambda1", "lambda2", "lambda3", "lambda4", "alpha", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=[0.95, -2.0, base[2], base[3], 1.0, base[5], base[6], base[7], base[8], base[9], base[10]],
            bounds=common_bounds,
            frozen_indices=[4],
            kernel_variant="centered_quad_logpair",
        ),
        CandidateSpec(
            name="xbar_quad_logpair_fixedbump",
            fit_name="BroadBump42XbarQuadLogPairFixedBump",
            np_name="NP-BroadBump42XbarQuadLogPairFixedBump.cl",
            param_names=["lambda1", "lambda2", "lambda3", "lambda4", "alpha", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=base,
            bounds=common_bounds,
            frozen_indices=[4, 5, 6],
...
def summarize_results(specs: dict[str, CandidateSpec]) -> pd.DataFrame:
    ...
    rows = [
        {
            "candidate": "xbar_quad_logpair_bump",
