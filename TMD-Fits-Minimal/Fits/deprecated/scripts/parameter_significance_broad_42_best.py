...
RESULTS_DIR = FITS_DIR / "parameter_significance_broad_42_best_results"
...
MODEL = ModelConfig(
    name="broad_bump_42_alpha1_best",
    label="Broad-bump 4-2 loggauss (alpha=1)",
    fit_name="BroadBump42LogGaussAlpha1Best",
    card_path=CARDS_DIR / "BroadBump42LogGaussAlpha1Best.jl",
    result_path=FITS_DIR / "broad_bump_42_results" / "loggauss_w060_alpha1.json",
)
...
def make_spec(model: ModelConfig) -> CandidateSpec:
    ...
    return CandidateSpec(
        ...
        kernel_variant="broad_42_significance",
    )
...
def analyze_model(model: ModelConfig, nested_maxfun: int, rel_step: float) -> None:
    ...
    strongest = nested_df.head(7).copy()
    ...
    lines = [
        "# Parameter Significance For Broad-Bump 4-2 Best Model",
        "",
        f"- Model: `{model.label}`",
        ...
        "Method:",
        "",
        "- Local finite-difference Hessian around the saved broad-bump 4-2 point.",
        "- PSD-projected covariance from the chi2 Hessian, on free parameters only.",
        "- Nested local refits with one broad-shape term or the bump block turned off.",
        "",
        "Saved files:",
        "",
