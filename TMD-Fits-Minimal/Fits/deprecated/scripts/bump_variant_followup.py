...
RESULTS_DIR = FITS_DIR / "bump_variant_followup_results"
RESULTS_DIR.mkdir(exist_ok=True)

BASE_RESULT = FITS_DIR / "pruned_model_refits_0_2_results" / "poly_bstar_cslog_loggauss_lambda3_0_alpha1.json"

DEFAULT_CANDIDATES = [
    "reduced_loggauss_refit",
    "reduced_logsech",
    "reduced_loglorentz",
    "reduced_loggauss_asym",
    "reduced_loggauss_power",
    "reduced_loggauss_power_asym",
]
...
def _render_kernel(spec: CandidateSpec) -> str:
    ...
    if variant == "reduced_loggauss_refit":
...
"""
  ...
  float invsig = 1.f / fmax(sigx, 1e-4f);
  float u = lx * invsig;
  float bump = amp * sechf(u);
  float shape = base + bump;
"""
    elif variant == "reduced_loglorentz":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  ...
  float invsig = 1.f / fmax(sigx, 1e-4f);
  float u = lx * invsig;
  float bump = amp / (1.f + u * u);
  float shape = base + bump;
"""
    elif variant == "reduced_loggauss_asym":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp, skew;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  const float lambda4 = p->lambda4;
  ...
  float lx = log(x) - logx0;
  float sigmaL = sigx * expf(-skew);
  float sigmaR = sigx * expf(skew);
  float sigma = lx < 0.f ? sigmaL : sigmaR;
  float u = lx / fmax(sigma, 1e-4f);
  float bump = amp * expf(-0.5f * u * u);
  float shape = base + bump;
"""
    elif variant == "reduced_loggauss_power":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp, pshape;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  const float lambda4 = p->lambda4;
  ...
  float lx = log(x) - logx0;
  float invsig = 1.f / fmax(sigx, 1e-4f);
  float u = fabs(lx * invsig);
  float bump = amp * expf(-0.5f * powr(fmax(u, 1e-6f), pshape));
  float shape = base + bump;
"""
    elif variant == "reduced_loggauss_power_asym":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp, skew, pshape;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  const float lambda4 = p->lambda4;
  ...
  float sigmaR = sigx * expf(skew);
  float sigma = lx < 0.f ? sigmaL : sigmaR;
  float u = fabs(lx / fmax(sigma, 1e-4f));
  float bump = amp * expf(-0.5f * powr(fmax(u, 1e-6f), pshape));
  float shape = base + bump;
"""
    else:
...
def followup_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            name="reduced_loggauss_refit",
            fit_name="ReducedLogGaussRefit",
            np_name="NP-ReducedLogGaussRefit.cl",
        ...
        CandidateSpec(
            name="reduced_logsech",
            fit_name="ReducedLogSech",
            np_name="NP-ReducedLogSech.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=common_init,
            bounds=common_bounds,
            frozen_indices=[],
            kernel_variant="reduced_logsech",
        ),
        CandidateSpec(
            name="reduced_loglorentz",
            fit_name="ReducedLogLorentz",
            np_name="NP-ReducedLogLorentz.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "BNP", "c0", "c1"],
            initial_params=common_init,
            bounds=common_bounds,
            frozen_indices=[],
            kernel_variant="reduced_loglorentz",
        ),
        CandidateSpec(
            name="reduced_loggauss_asym",
            fit_name="ReducedLogGaussAsym",
            np_name="NP-ReducedLogGaussAsym.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "skew", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="reduced_loggauss_asym",
        ),
        CandidateSpec(
            name="reduced_loggauss_power",
            fit_name="ReducedLogGaussPower",
            np_name="NP-ReducedLogGaussPower.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "pshape", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, 2.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="reduced_loggauss_power",
        ),
        CandidateSpec(
            name="reduced_loggauss_power_asym",
            fit_name="ReducedLogGaussPowerAsym",
            np_name="NP-ReducedLogGaussPowerAsym.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "skew", "pshape", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, 0.0, 2.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="reduced_loggauss_power_asym",
        ),
    ]
...
def summarize_results(specs: dict[str, CandidateSpec]) -> pd.DataFrame:
    ...
    for name in specs:
        ...
        row = {
            ...
            "delta_total_vs_reduced_loggauss": metrics["chi2dN_total"] - ref_metrics["chi2dN_total"],
            "delta_absdev_vs_reduced_loggauss": metrics["highE_mean_absdev_first3"] - ref_metrics["highE_mean_absdev_first3"],
            "delta_shortfall_vs_reduced_loggauss": metrics["highE_mean_shortfall_first3"] - ref_metrics["highE_mean_shortfall_first3"],
