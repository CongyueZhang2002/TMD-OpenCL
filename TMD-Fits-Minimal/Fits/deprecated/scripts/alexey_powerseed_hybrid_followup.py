...
RESULTS_DIR = FITS_DIR / "alexey_powerseed_hybrid_results"
RESULTS_DIR.mkdir(exist_ok=True)

POWERSEED_PATH = FITS_DIR / "bump_variant_followup_results" / "reduced_loggauss_powerseed.json"
ALEXEY_EXP_BUMP_PATH = FITS_DIR / "alexey_bias_family_results" / "alexey_bias_exp_loggauss.json"

DEFAULT_CANDIDATES = [
    "powerseed_den_full_sech",
    "powerseed_den_full_exp",
    "powerseed_den_base_sech",
    "powerseed_den_base_exp",
    "powerseed_den_linear_sech",
    "powerseed_den_linear_exp",
]


def render_kernel(spec: CandidateSpec) -> str:
    use_exp = spec.kernel_variant.endswith("_exp")
    base_only = "_base_" in spec.kernel_variant
    linear_only = "_linear_" in spec.kernel_variant

    if use_exp:
        mu_body = (
            "  float shape_eff = fmax(shape_mod, 1e-6f);\n"
            "  float SNP_mu = expf(-shape_eff * b2 / fmax(denom, 1e-12f));\n"
        )
    else:
        mu_body = "  float SNP_mu = sechf(shape_mod * b / fmax(denom, 1e-12f));\n"

    if linear_only:
        shape_mod = "broad_linear / fmax(denom, 1e-12f) + broad_log + bump"
    elif base_only:
        shape_mod = "base / fmax(denom, 1e-12f) + bump"
    else:
        shape_mod = "shape"
    denom_expr = "sqrtf(1.f + l0 * x * x * b2)"
...
{{
  ...
  float broad_linear = lambda1 * xbar + lambda2 * x;
  float broad_log = lambda4 * log(x);
  float base = broad_linear + broad_log;
  float u = (log(x) - logx0) / fmax(sigx, 1e-6f);
  float bump = amp * expf(-0.5f * u * u);
  float shape = base + bump;
  float denom = {denom_expr};
  float shape_mod = {shape_mod};
{mu_body}
...
def family_specs() -> dict[str, CandidateSpec]:
    powerseed = json.loads(POWERSEED_PATH.read_text(encoding="utf-8"))["best"]["full_params"]
    alexey_bump = json.loads(ALEXEY_EXP_BUMP_PATH.read_text(encoding="utf-8"))["best"]["full_params"] if ALEXEY_EXP_BUMP_PATH.exists() else None
    l0_init = float(alexey_bump[2]) if alexey_bump is not None else 1.0

    base_initial = [powerseed[0], powerseed[1], powerseed[2], powerseed[3], powerseed[4], powerseed[5], l0_init, powerseed[6], powerseed[7], powerseed[8]]
    bounds = [
        (0.02, 4.0),
        (0.02, 4.0),
        (-0.5, 0.5),
    ...
    names = ["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "l0", "BNP", "c0", "c1"]

    specs = {
        "powerseed_den_full_sech": CandidateSpec(
            name="powerseed_den_full_sech",
            fit_name="PowerseedDenFullSech",
            np_name="NP-PowerseedDenFullSech.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_full_sech",
        ),
        "powerseed_den_full_exp": CandidateSpec(
            name="powerseed_den_full_exp",
            fit_name="PowerseedDenFullExp",
            np_name="NP-PowerseedDenFullExp.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_full_exp",
        ),
        "powerseed_den_base_sech": CandidateSpec(
            name="powerseed_den_base_sech",
            fit_name="PowerseedDenBaseSech",
            np_name="NP-PowerseedDenBaseSech.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_base_sech",
        ),
        "powerseed_den_base_exp": CandidateSpec(
            name="powerseed_den_base_exp",
            fit_name="PowerseedDenBaseExp",
            np_name="NP-PowerseedDenBaseExp.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_base_exp",
        ),
        "powerseed_den_linear_sech": CandidateSpec(
            name="powerseed_den_linear_sech",
            fit_name="PowerseedDenLinearSech",
            np_name="NP-PowerseedDenLinearSech.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_linear_sech",
        ),
        "powerseed_den_linear_exp": CandidateSpec(
            name="powerseed_den_linear_exp",
            fit_name="PowerseedDenLinearExp",
            np_name="NP-PowerseedDenLinearExp.cl",
            param_names=names,
            initial_params=base_initial,
            bounds=bounds,
            frozen_indices=[],
            kernel_variant="powerseed_den_linear_exp",
        ),
    }
...
class HybridSession(FitSession):
    def candidate_start_points(self) -> list[np.ndarray]:
        ...
        anchor_values = {
            "lambda1": [0.05, 0.15, 0.35, 0.60, 0.85],
            "lambda2": [0.05, 0.15, 0.35, 0.60, 0.85],
            "lambda4": [0.10, 0.30, 0.50, 0.70],
            "logx0": [0.20, 0.40, 0.60, 0.80],
            "sigx": [0.10, 0.25, 0.45, 0.70],
            "amp": [0.15, 0.35, 0.65, 0.85],
            "l0": [0.00, 0.05, 0.15, 0.30, 0.55, 0.80],
    ...
    def fit(self, maxfun: int) -> dict:
        ...
        refine_budget = max(150, maxfun // 2)
        polish_budget = max(150, maxfun // 3)
...
def summarize(specs: dict[str, CandidateSpec]) -> pd.DataFrame:
    rows = []
    ref = json.loads(POWERSEED_PATH.read_text(encoding="utf-8"))["best"]["metrics"]
    rows.append(
        {
            "candidate": "powerseed",
    ...
    df = pd.DataFrame(rows).sort_values(["highE_mean_absdev_first3", "chi2dN_total"]).reset_index(drop=True)
...
def main() -> None:
    ...
    parser.add_argument("--maxfun", type=int, default=300)
