...
RESULTS_DIR = FITS_DIR / "xb_patch_followup_results"
...
def _base_best() -> list[float]:
    ...
    if "full_params" in block:
        return list(map(float, block["full_params"]))
    return list(map(float, block["free_params"]))
...
def _render_kernel(spec: CandidateSpec) -> str:
    if spec.kernel_variant == "xb_patch_logb_mult":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float logx1, sigx1, logb1, sigb, apatch;\n  float BNP, c0, c1;\n"
        body = """
  ...
  const float logx1 = p->logx1;
  const float sigx1 = p->sigx1;
  const float logb1 = p->logb1;
  const float sigb = p->sigb;
  const float apatch = p->apatch;
  ...
  float lx0 = log(x) - logx0;
  float ux0 = lx0 / fmax(sigx, 1e-4f);
  float wx0 = expf(-0.5f * ux0 * ux0);
  float bump = amp * wx0;

  float lx1 = log(x) - logx1;
  float ux1 = lx1 / fmax(sigx1, 1e-4f);
  float wx1 = expf(-0.5f * ux1 * ux1);
  ...
  float SNP_mu0 = sechf((base + bump) * b);
  float expo = clampf(apatch * wx1 * wb, -2.0f, 4.0f);
  float SNP_mu = SNP_mu0 * expf(-expo);
"""
    elif spec.kernel_variant == "xb_patch_peak_mult":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float logx1, sigx1, bw, apatch;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  const float lambda4 = p->lambda4;
  ...
  float z = b / fmax(bw, 1e-4f);
  float wb = z * z * expf(-0.5f * z * z);
...
def followup_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            name="xb_patch_logb_mult",
            fit_name="XBPatchLogBMult",
            np_name="NP-XBPatchLogBMult.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "logx1", "sigx1", "logb1", "sigb", "apatch", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, np.log(0.02), 0.35, np.log(1.5), 0.45, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="xb_patch_logb_mult",
        ),
        CandidateSpec(
            name="xb_patch_peak_mult",
            fit_name="XBPatchPeakMult",
            np_name="NP-XBPatchPeakMult.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "logx1", "sigx1", "bw", "apatch", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, np.log(0.02), 0.35, 0.8, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="xb_patch_peak_mult",
        ),
    ]
...
def main() -> None:
    ...
    parser.add_argument("--candidate", choices=DEFAULT_CANDIDATES)
    ...
    orchestrate(DEFAULT_CANDIDATES, maxfun=args.maxfun)
