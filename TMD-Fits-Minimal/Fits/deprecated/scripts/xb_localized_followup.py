...
RESULTS_DIR = FITS_DIR / "xb_localized_followup_results"
...
def _render_kernel(spec: CandidateSpec) -> str:
    if spec.kernel_variant == "xb_bump_logb_mult":
...
"""
    elif spec.kernel_variant == "xb_bump_logb_shape":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float logb0, sigb, axb;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  ...
  float shape = base + amp * wx + axb * wx * wb;
  float SNP_mu = sechf(shape * b);
"""
    elif spec.kernel_variant == "xb_bump_peak_shape":
        struct_fields = "  float lambda1, lambda2, lambda4;\n  float logx0, sigx, amp;\n  float bw, axb;\n  float BNP, c0, c1;\n"
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
...
def followup_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            ...
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "logb0", "sigb", "axb", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, np.log(0.75), 0.45, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="xb_bump_logb_mult",
        ),
        CandidateSpec(
            name="xb_bump_logb_shape",
            fit_name="XBBumpLogBShape",
            np_name="NP-XBBumpLogBShape.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "logb0", "sigb", "axb", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, np.log(0.75), 0.45, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
            ...
            frozen_indices=[],
            kernel_variant="xb_bump_logb_shape",
        ),
        CandidateSpec(
            name="xb_bump_peak_shape",
            fit_name="XBBumpPeakShape",
            np_name="NP-XBBumpPeakShape.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "amp", "bw", "axb", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, 0.8, 0.0, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="xb_bump_peak_shape",
        ),
    ]
