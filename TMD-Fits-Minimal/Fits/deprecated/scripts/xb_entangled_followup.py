...
RESULTS_DIR = FITS_DIR / "xb_entangled_followup_results"
...
def _render_kernel(spec: CandidateSpec) -> str:
    ...
    if variant == "entangle_window_exp":
        body = """
  ...
  float u = lx * invsig;
  float wx = expf(-0.5f * u * u);

  float rb = b / fmax(bw, 1e-4f);
  float hb = (b * b) / (1.f + rb * rb);

  float SNP_mu0 = sechf(base * b);
  float expo = clampf(axb * wx * hb, -2.0f, 4.0f);
  float SNP_mu = SNP_mu0 * expf(-expo);
"""
    elif variant == "entangle_inside_sech":
        body = """
  const float lambda1 = p->lambda1;
  const float lambda2 = p->lambda2;
  const float lambda4 = p->lambda4;
  ...
  float h1 = b / sqrtf(b * b + bw * bw);
  float shape = base + axb * wx * h1;
...
def followup_specs() -> list[CandidateSpec]:
    ...
    return [
        CandidateSpec(
            ...
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "axb", "bw", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, abs(amp), 0.7, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="entangle_window_exp",
        ),
        CandidateSpec(
            name="entangle_inside_sech",
            fit_name="EntangleInsideSech",
            np_name="NP-EntangleInsideSech.cl",
            param_names=["lambda1", "lambda2", "lambda4", "logx0", "sigx", "axb", "bw", "BNP", "c0", "c1"],
            initial_params=[lam1, lam2, lam4, logx0, sigx, amp, 0.7, BNP, c0, c1],
            bounds=[
                (0.02, 8.0),
                (0.02, 8.0),
                (-0.5, 0.5),
            ...
            frozen_indices=[],
            kernel_variant="entangle_inside_sech",
        ),
    ]
