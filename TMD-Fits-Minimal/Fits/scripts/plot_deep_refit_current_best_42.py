...
from deep_refit_current_best_42 import CURRENT_SPEC
from _paths import RESULTS_ROOT


OUT_DIR = RESULTS_ROOT / "model_comparison_plots" / "deep_refit_current_best_42"
DATA_ROOT = FITS_DIR.parent / "Data" / "Default" / "Cutted" / "DY"
RESULT_PATH = RESULTS_ROOT / "deep_refit_current_best_42_results" / "deep_refit_current_best_42.json"
...
def export_bins(files: list[str]) -> tuple[pd.DataFrame, dict]:
    ...
    full_params = np.asarray(result["best"]["full_params"], dtype=float)

    session = FitSession(CURRENT_SPEC)
