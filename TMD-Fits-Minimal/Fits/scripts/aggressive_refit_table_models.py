...
RESULTS_DIR = RESULTS_ROOT / "aggressive_table_refit_results"
...
MODELS = [
    BaseModel(
        ...
        result_json=RESULTS_ROOT / "art23_family_results" / "art23_mu_poly_bstar_cslog.json",
    ),
    BaseModel(
        ...
        result_json=RESULTS_ROOT / "localized_shape_followup_results" / "poly_bstar_cslog_loggauss.json",
    ),
    BaseModel(
        ...
        result_json=RESULTS_ROOT / "bump_variant_followup_results" / "reduced_loggauss_powerseed.json",
    ),
]
...
def reference_json_for(model: BaseModel, table_name: str) -> Path | None:
    if table_name == "MSHT20N3LO-MC-freeze":
        path = RESULTS_ROOT / "freeze_table_refit_results" / f"{model.name}__freeze.json"
        if path.exists():
            return path
        return model.result_json if model.result_json.exists() else None
    if table_name in {"MSHT20N3LO-MC-0-2", "MSHT20N3LO-MC-4-2"}:
        table_path = RESULTS_ROOT / "table_variant_refit_results" / f"{model.name}__{table_name}.json"
